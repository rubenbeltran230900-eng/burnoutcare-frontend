    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">{t('detail_loading')}</p>
        </div>
      </div>
    );
  }
  ...
  if (evaluaciones.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h2 className="text-xl font-semibold text-gray-600">{t('detail_no_evals_title')}</h2>
        <p className="text-gray-500 mt-2">{t('detail_no_evals_desc')}</p>
        <button
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">{t('detail_title')}</h1>
          <p className="text-gray-600">{t('detail_subtitle')}</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow p-4">
            <h2 className="font-semibold text-gray-800 mb-4">{t('detail_cases_list')}</h2>
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{ev.area || t('detail_no_area')}</p>
                      </div>
        ...
        <div className="lg:col-span-3">
          {evaluacionSeleccionada && (
            <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-600 mt-1">
                        <span className="flex items-center gap-1">
                          ...
                          {evaluacionSeleccionada.area || t('detail_no_area')}
                        </span>
                        <span className="flex items-center gap-1">
                          <Briefcase className="w-4 h-4" />
                          {evaluacionSeleccionada.puesto || t('detail_no_position')}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className={`px-4 py-2 rounded-lg ${getColorRiesgo(evaluacionSeleccionada.nivel_riesgo).bg}`}>
                    <p className="text-xs text-gray-600">{t('risk_level')}</p>
              ...
              {/* Resultados CBI */}
              <div className="bg-white rounded-xl shadow p-6">
                <h3 className="font-semibold text-gray-800 mb-4">{t('detail_cbi_results')}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className={`p-4 rounded-lg border-2 ${getColorRiesgo(determinarNivel(evaluacionSeleccionada.puntaje_bp)).border} ${getColorRiesgo(determinarNivel(evaluacionSeleccionada.puntaje_bp)).bg}`}>
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-medium text-gray-700">{t('dim_personal')}</h4>
                    ...
                    <p className="text-sm text-gray-600 mt-1">{t('detail_nivel')} {evaluacionSeleccionada.nivel_bp}</p>
                  ...
                  <div className={`p-4 rounded-lg border-2 ${getColorRiesgo(determinarNivel(evaluacionSeleccionada.puntaje_bl)).border} ${getColorRiesgo(determinarNivel(evaluacionSeleccionada.puntaje_bl)).bg}`}>
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-medium text-gray-700">{t('dim_work')}</h4>
                    ...
                    <p className="text-sm text-gray-600 mt-1">{t('detail_nivel')} {evaluacionSeleccionada.nivel_bl}</p>
                    <div className="mt-3 bg-white bg-opacity-50 rounded p-2">
                      <p className="text-xs text-gray-600">
                        {t('detail_bl_desc')}
                      </p>
                  ...
                  <div className={`p-4 rounded-lg border-2 ${getColorRiesgo(determinarNivel(evaluacionSeleccionada.puntaje_bc)).border} ${getColorRiesgo(determinarNivel(evaluacionSeleccionada.puntaje_bc)).bg}`}>
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-medium text-gray-700">{t('dim_client')}</h4>
                    ...
                    <p className="text-sm text-gray-600 mt-1">{t('detail_nivel')} {evaluacionSeleccionada.nivel_bc}</p>
                    <div className="mt-3 bg-white bg-opacity-50 rounded p-2">
                      <p className="text-xs text-gray-600">
                        {t('detail_bc_desc')}
                      </p>
                ...
                <div className="mt-6 space-y-3">
                  <h4 className="text-sm font-medium text-gray-700">{t('detail_scores_viz')}</h4>

                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      <span className="text-sm text-gray-600 w-32">{t('dim_personal')} (BP)</span>
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-sm text-gray-600 w-32">{t('dim_work')} (BL)</span>
                      <div className="flex-1 bg-gray-200 rounded-full h-4">
                        <div
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-sm text-gray-600 w-32">{t('dim_client')} (BC)</span>
                      <div className="flex-1 bg-gray-200 rounded-full h-4">
                        <div
              ...
              {historial.length > 1 && (
                <div className="bg-white rounded-xl shadow p-6">
                  <h3 className="font-semibold text-gray-800 mb-4">{t('detail_history')}</h3>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b">
                          <th className="text-left py-2 px-3">{t('alerts_col_date')}</th>
                          <th className="text-center py-2 px-3">BP</th>
                          <th className="text-center py-2 px-3">BL</th>
                          <th className="text-center py-2 px-3">BC</th>
                          <th className="text-center py-2 px-3">{t('risk_level')}</th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          <tr key={ev.id} className={`border-b ${idx === 0 ? 'bg-blue-50' : ''}`}>
                            <td className="py-2 px-3">
                              <div className="flex items-center gap-2">
                                ...
                                {idx === 0 && <span className="text-xs bg-blue-200 text-blue-800 px-2 rounded">{t('detail_current')}</span>}
                              </div>
                            </td>
